...
function VisionScanner({ isOpen, onClose, t = {}, darkMode }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={handleCloseModal}>
      <div
        className={`rounded-2xl shadow-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto border relative flex flex-col ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-900'
          }`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className={`sticky top-0 z-30 px-6 py-4 border-b flex items-center justify-between shadow-md ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
          }`}>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <label className={`block text-center p-5 border-2 border-dashed rounded-2xl cursor-pointer transition hover:scale-[1.02] ${darkMode ? 'border-slate-700 hover:border-emerald-500 bg-slate-950' : 'border-slate-300 hover:border-emerald-600 bg-slate-50'
              }`}>
            ...
            <button
              ...
              className={`text-center p-5 border-2 rounded-2xl cursor-pointer transition hover:scale-[1.02] ${cameraActive
            ...
            <div className="flex flex-wrap gap-2">
              {samplePresets.map((preset) => (
                <button
                  ...
                  className={`text-xs font-semibold px-3 py-2 rounded-xl border transition flex items-center gap-1 cursor-pointer ${darkMode
          ...
          {scanResult && scanResult.detectedItem && (
            <div className={`p-5 rounded-2xl border shadow-xl transition-all animate-fadeIn ${scanResult.isValid
              ...
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <span className={`font-bold text-xs px-3 py-1.5 rounded-full uppercase tracking-wider ${scanResult.isValid ? 'bg-emerald-600 text-white' : 'bg-red-600 text-white'
                  }`}>
        ...
        {/* Footer */}
        <div className={`sticky bottom-0 px-6 py-3.5 border-t flex justify-end ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
          }`}>
          <button
            type="button"
            onClick={handleCloseModal}
      ...
      <div className="flex flex-wrap gap-2">
        {sources.map((src, idx) => (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border font-medium transition text-xs sm:text-sm ${darkMode
...
function ComplianceCheck({ isOpen, onClose, t = {}, darkMode }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className={`rounded-2xl shadow-2xl max-w-xl w-full p-6 sm:p-8 border relative ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-900'
          }`}
        onClick={(e) => e.stopPropagation()}
      >
        <button
          type="button"
          onClick={onClose}
          className={`absolute top-4 right-4 p-2 rounded-lg transition font-bold text-xl ${darkMode ? 'text-slate-400 hover:text-slate-100 hover:bg-slate-800' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100'
            }`}
        >
          ✕
        </button>
        ...
        <form onSubmit={handleSearch} className="mb-6">
          <div className="flex gap-2">
            <input
              ...
              className={`flex-1 px-4 py-3 border-2 rounded-xl focus:outline-none focus:border-orange-500 text-lg ${darkMode ? 'bg-slate-950 border-slate-700 text-slate-100 placeholder-slate-500' : 'bg-slate-50 border-slate-300 text-slate-900'
                }`}
              autoFocus
            />
            <button
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2">
          ...
          {['Helmet', 'Packaged Water', 'Pressure Cooker', 'Toys', 'LED Bulb', 'Gold'].map((item) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg border transition font-medium ${darkMode ? 'bg-slate-800 text-slate-300 hover:bg-slate-700 border-slate-700' : 'bg-slate-100 text-slate-700 hover:bg-orange-50 hover:text-orange-700 border-slate-200'
                }`}
            >
              {item}
            </button>
...
function InputBar({ onSend, loading, t = {}, lang, darkMode }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-4xl mx-auto">
      <div className={`relative flex items-center rounded-2xl shadow-lg border-2 transition p-2 ${darkMode
        ...
        <input
          ...
          disabled={loading}
          className={`flex-1 px-4 py-3 text-lg md:text-xl bg-transparent focus:outline-none min-h-[54px] ${darkMode ? 'text-slate-100 placeholder-slate-500' : 'text-slate-900 placeholder-slate-400'
            }`}
        />
        ...
        <div className="flex items-center gap-2 pr-1">
          <button
            ...
            className={`p-3 rounded-xl transition flex items-center justify-center text-xl ${isListening
...
function ChatBox({ messages, loading, t = {}, darkMode, lang, onClear, onOpenChatPDF }) {
  ...
  return (
    <div className={`rounded-2xl shadow-md border p-4 sm:p-6 mb-6 flex flex-col h-[520px] relative overflow-hidden transition-colors ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-[#f8fafc] border-slate-200'
      }`}>
      {messages && messages.length > 0 && (
        <div className="flex justify-end mb-2 shrink-0">
          <button
            ...
            className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition flex items-center gap-1.5 cursor-pointer ${darkMode
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-6 relative z-10">
        {(!messages || !Array.isArray(messages) || messages.length === 0) ? (
          ...
          messages.map((msg, idx) => (
            ...
            >
              ...
              <div
                className={`max-w-[85%] sm:max-w-[78%] rounded-2xl p-4 sm:p-5 shadow-sm border text-lg relative ${msg.type === 'user'
                  ...
                  {msg.type === 'bot' && (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`text-xs font-bold px-2 py-1 rounded-lg border transition flex items-center gap-1 cursor-pointer ${speakingIdx === idx
                      ...
                      <button
                        ...
                        className={`text-xs font-bold px-2 py-1 rounded-lg border transition flex items-center gap-1 cursor-pointer ${darkMode ? 'bg-emerald-950/70 hover:bg-emerald-900 text-emerald-300 border-emerald-800' : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border-emerald-300'
                          }`}
        ...
        {loading && (
          <div className={`flex gap-4 items-center p-4 rounded-2xl border text-lg ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-700'
            }`}>
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between transition-colors duration-200 relative ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-100 text-slate-900'
      }`}>

      <button
        type="button"
        onClick={(e) => { e.preventDefault(); setDarkMode(!darkMode); }}
        className={`fixed top-4 right-4 z-50 p-3 rounded-full shadow-2xl border transition-all flex items-center justify-center cursor-pointer text-xl hover:scale-110 active:scale-95 ${darkMode
      ...
      <main className="max-w-5xl mx-auto w-full px-4 py-6 flex-1">
        <div className={`rounded-2xl p-4 shadow-md border mb-6 flex flex-wrap items-center justify-between gap-4 ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
          }`}>
          ...
          <div className="flex items-center gap-2 flex-1 max-w-xs">
            <select
              ...
              className={`w-full border-2 rounded-xl px-4 py-2 font-semibold focus:outline-none text-lg cursor-pointer ${darkMode
        ...
        <div className="mb-4">
          <div className={`flex items-center justify-between gap-2 font-bold text-sm mb-2 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${darkMode ? 'bg-emerald-950 text-emerald-300 border border-emerald-800' : 'bg-emerald-100 text-emerald-800'
                }`}>
                {category}
              </span>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {Array.isArray(currentSampleQuestions) && currentSampleQuestions.map((q, idx) => (
              <button
                ...
                className={`text-sm sm:text-base font-semibold px-4 py-2 rounded-xl border shadow-sm transition-all text-left cursor-pointer ${darkMode
      ...
      <footer className={`py-4 px-4 text-center border-t text-sm mt-8 ${darkMode ? 'bg-slate-950 text-slate-500 border-slate-900' : 'bg-slate-900 text-slate-400 border-slate-800'
        }`}>
